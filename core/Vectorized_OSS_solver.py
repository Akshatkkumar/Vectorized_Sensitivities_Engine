...
def gen_gbm_oss_jax(S_0, r, b, v, ts, B, S_ref, key):
    ...
    def step_fn(carry, inputs):
        ...
        dt = inputs
        # survival prob.
        p = norm.cdf((log_barrier - X_prev - mu_p * dt) / (v * jnp.sqrt(dt)))
        # draw from truncated normal
        u = jax.random.uniform(current_key)
        z = norm.ppf(u * p)
        X_next = X_prev + (mu_p) * dt + v * jnp.sqrt(dt) * z
...
#Note to self: Grad Exp[F(THETA)] = Exp[Grad F(THETA)] used to vmap AAD on single path two all paths. This allows for mean and std calculation
def get_delta_aad_jax(S_0: float, r: float, b: float, v: float, Qs: jnp.ndarray, ts: jnp.ndarray, B: float, S_ref: float, N: int, key: jax.random.PRNGKey) -> float:
    keys = jax.random.split(key, N)
    # Wrapper for AAD on one path
    def single_path_value(s_start, k):
        Xs, Ps = gen_gbm_oss_jax(s_start, r, b, v, ts, B, S_ref, k)
        return value_oss_jax(Xs, Ps, Qs, ts, s_start, S_ref, r)
    grad_single = jax.grad(single_path_value)
    # vmap to all paths
    pathwise_deltas = jax.vmap(grad_single, in_axes=(None, 0))(S_0, keys)
...
estimated_deltas = get_delta_aad_jax(S_0, r, b, v_fixed, Qs, ts, B, S_ref, N, d_key)
val_mean, val_std = jnp.mean(estimated_values), jnp.std(estimated_values)
delta_mean, delta_std = jnp.mean(estimated_deltas), jnp.std(estimated_deltas)
...
plt.suptitle("MC+OSS+XLA/AAD Simulation Results")
plt.show()